...
import { EmailTemplate } from "@/emails/evening"
...
async function sendMorningEmail() {
  try {
    ...
    const tomorrow = await getTasksPreview(userId, {
      hideCompleted: true,
      to: dayjs().endOf("day").add(1, "day").toISOString(),
    })
    ...
    const completedToday = await getTasksPreview(userId, {
      statuses: ["ARCHIVE"],
      completedFrom: dayjs().startOf("day").toISOString(),
      completedTo: dayjs().endOf("day").toISOString(),
    })
    ...
    const feedbackPrompt = `
      ...
      Every evening, you'll get a list of tasks that was due today, tasks that are overdue, and tasks that were completed today.
      ...
      The feedback should be very specific about the user tasks, and should help him finish his day the best way possible.
      ...
      messages: [
        {
          ...
          - Stop trying to control your time. Start live it and be present
            ...
            Please help the user end his day the best way possible.
    ...
    const data = await resend.emails.send({
      from: "Stay Current <evening@staycurrent.app>",
      to: ["eyalcohen4.ec@gmail.com"],
      subject: `Your Current Evening: ${dayjs().format("MMMM D, YYYY")} Report`,
      react: EmailTemplate({
        firstName: user?.name || "",
